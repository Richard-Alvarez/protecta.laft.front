...
export class RecurseProfile implements OnInit {
  ...
  async setArrayCabecera() {
    ...
    this.arrayCabecera = this.arrayCabecera.concat(
      this.arrayResponseProfile.map((t) => t.profileName)
    );
  }

  async search(parameters) {
    let response = await this.userconfigService.getListaRecurseProfile(
      parameters
    );
    response = response.filter((t, i, array) => {
      t.arrayIdProfile = array.filter(
        (t2) => t2.nIdResource == t.nIdResource && t2.nIdFather == t.nIdFather
      );
      return (
        array
          .map((m) => m.nIdResource + "," + m.nIdFather)
          .indexOf(t.nIdResource + "," + t.nIdFather) === i
      );
    });
    response.forEach((t) => {
      t.profile = [];
      this.arrayResponseProfile.forEach((p) => {
        let res = {
          isChecked: t.arrayIdProfile
            .map((t) => t.nIdProfile)
            .includes(p.profileId),
    ...
    response.sort((a, b) => {
      return a.nIdResource - b.nIdResource;
    });
  ...
  async searchHistory(idProfile) {
    ...
    this.arrayResponseHistory =
      await this.userconfigService.getListaRecurseProfileHistory(data);
  }
  onChange(item) {
    var item2 = this.arrayRequestParameters.filter(
      (t) =>
        t.nIdResource == item.nIdResource &&
        t.nIdProfile == item.nIdProfile &&
        t.nIdFather == item.nIdFather
    );
    if (item2.length == 0) this.arrayRequestParameters.push(item);
    else {
      this.arrayRequestParameters = this.arrayRequestParameters.filter(
        (t, i, array) => {
  ...
  getExcel() {
    ...
    if (this.arrayRecurse.length > 0) {
      ...
      this.excelService.exportAsExcelFile(
        dataprepare,
        "Registros de recursos y perfiles"
      );
    } else
      swal
        .fire({
  ...
  getExcelHistory() {
    if (this.arrayResponseHistory.length > 0) {
      ...
      this.excelService.exportAsExcelFile(
        data,
        "Registros del historial de recursos y perfiles"
      );
    } else
      swal
        .fire({
  ...
  textCabecera(index) {
    if (this.arrayCabecera[0] == "Menú" || this.arrayCabecera[1] == "Submenú") {
      ...
      this.arrayCabecera.name == "Menú" ||
      this.arrayCabecera.name == "Submenú"
    ) {
      return "text-left";
    } else {
      return "text-center";
