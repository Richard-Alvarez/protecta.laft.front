...
CrearPdf(item) {
  ...
  DescargarPDFMixto(item){
    this.spinner.show();
    this.convertirPdfMixta(item,1)
    this.convertirPdfMixta(item,2)
    this.convertirPdfMixta(item,3)
    this.spinner.hide();
  }

  dataUser:any = {}
  convertirPdfMixta(item, validarImg) {
  ...
  GenerarPDFIndividual(item){
    ...
    this.convertirPdfIndividual(item,2)
    this.convertirPdfIndividual(item,3)
  }
    ...
    .then((pdf) => {
      ...
      for (var i = 1; i <= totalPages; i++) {
        ...
        if(validador == 1){
          ...
          pdf.addImage(imgRn, 'JPEG', 6.6, 10.5);
        }else if(validador == 3){
          pdf.addImage(imgIdecon, 'JPEG', 5.1, 10.6,2.5,0.5);
        }
